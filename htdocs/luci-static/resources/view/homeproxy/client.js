...
return view.extend({
	...
	render(data) {
		...
		so.validate = function (section_id) {
			...
			if (section_id && this.section.formvalue(section_id, 'node') === 'urltest' && !value.length)
				return _('Expecting: %s').format(_('non-empty value'));
		...
		so.validate = function (section_id) {
			...
			if (section_id && this.section.formvalue(section_id, 'node') === 'selector' && !value.length)
				return _('Expecting: %s').format(_('non-empty value'));
		...
		so = ss.option(hp.CBIStaticList, 'fallback_nodes', _('Fallback nodes'),
			_('List of nodes to fallback.'));
		for (let i in proxy_nodes)
			so.value(i, proxy_nodes[i]);
		so.depends('node', 'fallback');
		so.validate = function (section_id) {
			let value = this.section.formvalue(section_id, 'fallback_nodes');
			if (section_id && this.section.formvalue(section_id, 'node') === 'fallback' && !value.length)
				return _('Expecting: %s').format(_('non-empty value'));
		...
		so = ss.option(form.Value, 'fallback_url', _('Test URL'),
			_('The URL to test.'));
		so.placeholder = 'https://www.gstatic.com/generate_204';
		so.validate = function (section_id, value) {
			if (section_id && value) {
		...
		so = ss.option(form.Value, 'fallback_interval', _('Test interval'),
			_('The test interval in seconds.'));
		so.datatype = 'uinteger';
		so.placeholder = '300';
		so.validate = function (section_id, value) {
			if (section_id && value) {
				let idle_timeout = this.section.formvalue(section_id, 'fallback_idle_timeout') || '1800';
				if (parseInt(value) > parseInt(idle_timeout))
					return _('Test interval must be less or equal than idle timeout.');
			}
		...
		so = ss.option(form.Value, 'fallback_idle_timeout', _('Idle timeout'),
			_('The idle timeout in seconds.'));
		so.datatype = 'uinteger';
		so.placeholder = '1800';
		so.depends('node', 'fallback');
		...
		ss.tab('clash_api', _('Clash API'));

		so = ss.taboption('clash_api', form.Value, 'clash_api_port', _('External controller port'),
			_('RESTful API port for Clash.'));
		so.datatype = 'port';
		so.placeholder = '9090';

		so = ss.taboption('clash_api', form.Value, 'clash_api_ui', _('External UI path'),
			_('Path to the Clash Dashboard UI.'));
		so.placeholder = '/usr/share/homeproxy/ui';

		so = ss.taboption('clash_api', form.Value, 'clash_api_secret', _('API Secret'),
			_('Secret for the RESTful API.'));
		so.password = true;

		so = ss.taboption('clash_api', form.DummyValue, '_open_ui');
		so.render = function () {
			let port = this.section.formvalue('control', 'clash_api_port') || '9090';
			let hosts = window.location.host;
			let url = 'http://' + hosts.split(':')[0] + ':' + port + '/ui';
			return E('div', {}, [
				E('a', {
